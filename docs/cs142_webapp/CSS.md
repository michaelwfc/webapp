...
By default, every element has `position: static`. In static positioning, the browser places elements automatically in the normal document flow — one after another. The `left` and `top` CSS properties are completely ignored for static elements.

```css
/* ❌ Does nothing — static elements ignore left/top */
#div1 {
  left: 100px;
  top: 50px;
}
...
Once you set position to anything other than static (absolute, relative, or fixed), the element is taken out of the automatic flow and left/top become active:

```css
/* ✅ Now left/top actually move the element */
#div1 {
  position: absolute;
  left: 100px;
  top: 50px;
